...
- **Fix in progress — external pinger (cron-job.org), not a Claude scheduled task:** Seb wants something independent of his laptop and of Cowork being open (laptop sleeps most of the time) — he pointed out GitHub already has a recurring-task mechanism (`schedule:`), which is true; the actual problem is GitHub's own scheduler dropping runs, not a laptop/Cowork dependency in the existing setup. Agreed plan: **cron-job.org**, a free third-party cron-ping service running on its own servers, hitting the GitHub Actions dispatch API directly (`POST /repos/smatiere/matierehub/actions/workflows/{id}/dispatches`) — sidesteps GitHub's internal `schedule:` queue entirely.
  - New fine-grained GitHub PAT created, scoped to ONLY `smatiere/matierehub`, ONLY "Actions: Read and write" (narrower than the broad PAT Claude uses for code pushes, since a third-party service holds it). Verified working 2026-06-18 (200 on repo read, 204 on a real dispatch call). Value stored in Claude's memory only (`reference_github_actions_pat`) — never written into this repo, since it's public.
  - Seb configures the two cron-job.org jobs himself (Claude does not enter API tokens into third-party form fields). See `CLAUDE.md` → "External scheduling (cron-job.org)" for the exact endpoints/headers/body.
- **Next check:** once the cron-job.org jobs are live, verify via GitHub Actions run history (`event=workflow_dispatch` from the external pinger's IP/timing) or Supabase row growth — don't assume success from the job dashboard showing "200 OK" alone.
