...
**[FIXED]** GitHub Actions `schedule` trigger still not firing — off-hour+backup cron fix's first eligible day (06-17) had ZERO scheduled runs
...
- **Fix — external pinger (cron-job.org), not a Claude scheduled task:** Seb wanted something independent of his laptop and of Cowork being open (laptop sleeps most of the time) — the actual problem is GitHub's own scheduler dropping runs, not a laptop/Cowork dependency in the existing setup. Solution: **cron-job.org**, a free third-party cron-ping service running on its own servers, hitting the GitHub Actions dispatch API directly (`POST /repos/smatiere/matierehub/actions/workflows/{id}/dispatches`) — sidesteps GitHub's internal `schedule:` queue entirely.
  - New fine-grained GitHub PAT created, scoped to ONLY `smatiere/matierehub`, ONLY "Actions: Read and write" (narrower than the broad PAT Claude uses for code pushes, since a third-party service holds it). Value stored in Claude's memory only (`reference_github_actions_pat`) — never written into this repo, since it's public.
  - Seb configured the two cron-job.org jobs himself (Claude does not enter API tokens into third-party form fields), following exact endpoint/header/body specs from `CLAUDE.md` → "External scheduling (cron-job.org)". Job IDs: `7851996` (Daily Xero Sync), `7852039` (Weekly Dashboard Sync).
- **Verified working end-to-end 2026-06-18:** used cron-job.org's own "TEST RUN" feature (fires the configured request immediately, on demand) on both jobs. Both got `204 No Content` from GitHub with `x-accepted-github-permissions: actions=write`, confirming the PAT/headers Seb entered are correct. Cross-checked independently via the GitHub Actions API — matching `workflow_dispatch` runs appeared and completed `success` at the same timestamps: daily run `27754725605` at 10:57:15Z, weekly run `27754804172` at 10:58:48Z. Full chain (cron-job.org → GitHub dispatch → Actions run → Supabase write) confirmed live, independent of GitHub's internal scheduler and of Seb's laptop being on.
- **Belt-and-suspenders setup, by design:** cron-job.org pings run *alongside* the existing GitHub-internal `schedule:` crons, not replacing them — both syncs are idempotent upserts, so redundant triggers just mean more independent chances to fire with zero risk of duplicate data.
